...
cluster_fig_dir <- file.path(output_dir, "cluster_patient_stay_timelines")
...
for (i in seq_len(nrow(cluster_keys))) {
  ...
  genome_n <- cluster_keys$genome_n[i]
  patient_n <- cluster_keys$patient_n[i]

  cluster_samples <- clusters_plot %>%
  ...
  stay_df <- locations %>%
    filter(clade_group == clade_label, PatientID %in% patient_order$patient_id) %>%
    transmute(
  ...
  collection_plot_df <- cluster_samples_plot %>%
    mutate(
      collection_unit = if_else(is.na(collection_unit), "Unknown", collection_unit)
    )
  ...
  plot_subtitle <- paste0("n=", genome_n, " genomes across ", patient_n, " patients")
  ...
  file_stub <- paste0(clade_stub, "_cluster_", cluster_id_value, "_patient_stay_timeline")
...
write.csv(
  ...
  file.path(desc_dir, "cluster_patient_timeline_patient_label_lookup.csv"),
  row.names = FALSE
)
...
writeLines(
  c(
    ...
    "Y-axis patient labels are simplified to pt1, pt2, ... within each cluster.",
    "Additional one-day location rows from unmapped_samples_hosp_onset_noPHI.csv are included when sample_id matches the cleaned clade 1/2 metadata.",
  ...
  con = file.path(desc_dir, "cluster_patient_stay_timelines_notes.txt")
)

cat("Done. Cluster patient stay timeline figures written to:\n", cluster_fig_dir, "\n", sep = "")
