...
cluster_fig_dir <- file.path(output_dir, "cluster_patient_stay_timelines_location_only_same_location_anytime_links")
...
find_same_unit_anytime_links <- function(stay_df, patient_levels) {
  if (nrow(stay_df) < 2) {
    return(tibble(
      Unit = character(),
      patient_a = character(),
  ...
  split(stay_df, stay_df$Unit) %>%
    purrr::imap_dfr(function(unit_df, unit_name) {
      patients <- sort(unique(unit_df$patient_id))
      ...
      combn(patients, 2, simplify = FALSE) %>%
        map_dfr(function(pair_ids) {
          patient_a <- pair_ids[1]
          patient_b <- pair_ids[2]

          patient_a_rows <- unit_df %>%
            filter(patient_id == patient_a) %>%
            arrange(StartDate_date, EndDate_date)
          patient_b_rows <- unit_df %>%
            filter(patient_id == patient_b) %>%
            arrange(StartDate_date, EndDate_date)

          anchor_date <- min(
            c(patient_a_rows$StartDate_date, patient_b_rows$StartDate_date),
            na.rm = TRUE
          )
...
for (i in seq_len(nrow(cluster_keys))) {
  ...
  same_unit_links <- find_same_unit_anytime_links(stay_df, patient_order)
  ...
  plot_subtitle <- paste0(
    ...
    if (nrow(same_unit_links) > 0) paste0("; ", nrow(same_unit_links), " same-unit anytime links shown") else ""
  )

  all_dates <- c(stay_df$StartDate_date, stay_df$EndDate_date, collection_plot_df$collection_date)
  all_dates <- all_dates[!is.na(all_dates)]
  cluster_min_date <- min(all_dates)
  cluster_max_date <- max(all_dates)

  if (nrow(same_unit_links) > 0) {
    same_unit_links <- same_unit_links %>%
      arrange(patient_plot_id_a, patient_plot_id_b, Unit) %>%
      mutate(
        bracket_idx = row_number(),
        x_bracket = cluster_min_date - (bracket_idx * 4 + 2),
        x_tick_end = cluster_min_date - 1
      )
  }
  ...
  if (nrow(same_unit_links) > 0) {
    p <- p +
      geom_segment(
        ...
        aes(x = x_bracket, xend = x_bracket, y = patient_plot_id_a, yend = patient_plot_id_b, color = Unit),
        linetype = "dashed",
        linewidth = 0.8,
        alpha = 0.8
      ) +
      geom_segment(
        data = same_unit_links,
        aes(x = x_bracket, xend = x_tick_end, y = patient_plot_id_a, yend = patient_plot_id_a, color = Unit),
        linetype = "dashed",
        linewidth = 0.8,
        alpha = 0.8
      ) +
      geom_segment(
        data = same_unit_links,
        aes(x = x_bracket, xend = x_tick_end, y = patient_plot_id_b, yend = patient_plot_id_b, color = Unit),
        linetype = "dashed",
        linewidth = 0.8,
        alpha = 0.8
      )
  ...
  p <- p +
    ...
    scale_x_date(
      limits = c(
        if (nrow(same_unit_links) > 0) min(same_unit_links$x_bracket) - 2 else cluster_min_date - 2,
        cluster_max_date + 2
      )
    ) +
  ...
  file_stub <- paste0(clade_stub, "_cluster_", cluster_id_value, "_patient_stay_timeline_location_only_same_location_anytime_links")
...
write.csv(
  ...
  file.path(desc_dir, "cluster_patient_timeline_patient_label_lookup_location_only_same_location_anytime_links.csv"),
  row.names = FALSE
)

write.csv(
  bind_rows(link_summary_tables),
  file.path(desc_dir, "cluster_patient_same_location_anytime_links_summary.csv"),
  row.names = FALSE
)
...
writeLines(
  c(
    ...
    "Dashed vertical lines connect patients who ever share the same Unit within that cluster, regardless of time overlap.",
    "The dashed link is placed at the earliest observed start date across the linked patients in that unit.",
  ...
  con = file.path(desc_dir, "cluster_patient_stay_timelines_location_only_same_location_anytime_links_notes.txt")
)

cat("Done. Cluster patient stay timeline figures with same-location-anytime links written to:\n", cluster_fig_dir, "\n", sep = "")
