...
cluster_fig_dir <- file.path(output_dir, "cluster_patient_stay_timelines_location_only_no_special_units")
...
for (i in seq_len(nrow(cluster_keys))) {
  ...
  plot_subtitle <- paste0(
    ...
    if (removed_patient_n > 0) paste0(" (", removed_patient_n, " patients without non-special-unit location data removed)") else ""
  )
  ...
  file_stub <- paste0(clade_stub, "_cluster_", cluster_id_value, "_patient_stay_timeline_location_only_no_special_units")
...
write.csv(
  ...
  file.path(desc_dir, "cluster_patient_timeline_patient_label_lookup_location_only_no_special_units.csv"),
  row.names = FALSE
)
...
writeLines(
  c(
    ...
    "This location-only version removes patients without any non-special-unit location rows in the filtered location dataset within that window.",
  ...
  con = file.path(desc_dir, "cluster_patient_stay_timelines_location_only_no_special_units_notes.txt")
)

cat("Done. Cluster patient stay timeline no-special-unit figures written to:\n", cluster_fig_dir, "\n", sep = "")
