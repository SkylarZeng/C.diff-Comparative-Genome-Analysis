...
cluster_fig_dir <- file.path(output_dir, "cluster_patient_stay_timelines_location_only_same_location_links")
...
find_same_unit_links <- function(stay_df, patient_levels) {
  if (nrow(stay_df) < 2) {
    return(tibble(
      Unit = character(),
      patient_a = character(),
...
global_unit_palette <- c(global_unit_palette, "Unknown" = "#4d4d4d")
...
for (i in seq_len(nrow(cluster_keys))) {
  ...
  same_unit_links <- find_same_unit_links(stay_df, patient_order)
  ...
  plot_subtitle <- paste0(
    ...
    if (nrow(same_unit_links) > 0) paste0("; ", nrow(same_unit_links), " same-unit overlap links shown") else ""
  )
  ...
  file_stub <- paste0(clade_stub, "_cluster_", cluster_id_value, "_patient_stay_timeline_location_only_same_location_links")
...
write.csv(
  ...
  file.path(desc_dir, "cluster_patient_timeline_patient_label_lookup_location_only_same_location_links.csv"),
  row.names = FALSE
)

write.csv(
  bind_rows(link_summary_tables),
  file.path(desc_dir, "cluster_patient_same_location_links_summary.csv"),
  row.names = FALSE
)
...
writeLines(
  c(
    ...
    "Dashed vertical lines connect patients who have overlapping stays in the same Unit within that cluster.",
  ...
  con = file.path(desc_dir, "cluster_patient_stay_timelines_location_only_same_location_links_notes.txt")
)

cat("Done. Cluster patient stay timeline figures with same-location links written to:\n", cluster_fig_dir, "\n", sep = "")
