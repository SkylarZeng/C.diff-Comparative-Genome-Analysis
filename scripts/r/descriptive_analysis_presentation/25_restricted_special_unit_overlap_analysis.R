...
compute_restricted_flags <- function(snv_pairs, clade_label) {
  ...
  pair_flags <- snv_pairs %>% distinct(patient_a, patient_b)
  ...
  facility_overlap <- find_overlap_pairs_for_level(loc_clade, c("FacilityCode")) %>% mutate(facility_overlap = TRUE)
  floor_overlap <- find_overlap_pairs_for_level(loc_clade, c("floor")) %>% mutate(floor_overlap = TRUE)
  unit_overlap <- find_overlap_pairs_for_level(loc_clade, c("FacilityCode", "Unit")) %>% mutate(unit_overlap = TRUE)
  room_overlap <- find_overlap_pairs_for_level(loc_clade, c("FacilityCode", "Room")) %>% mutate(room_overlap = TRUE)
  pair_flags %>%
...
build_plot_data <- function(dat) {
  bind_rows(
    dat %>% filter(hospital_time_overlap) %>% transmute(clade_group, snv, exposure_type = "Facility", exposure_status = if_else(facility_overlap, "Overlap yes", "Overlap no")),
    dat %>% filter(hospital_time_overlap) %>% transmute(clade_group, snv, exposure_type = "Unit", exposure_status = if_else(unit_overlap, "Overlap yes", "Overlap no")),
    dat %>% filter(hospital_time_overlap) %>% transmute(clade_group, snv, exposure_type = "Room", exposure_status = if_else(room_overlap, "Overlap yes", "Overlap no"))
  ) %>%
    mutate(exposure_type = factor(exposure_type, levels = c("Facility", "Unit", "Room")))
}
...
make_distribution_plot <- function(plot_dat) {
  p <- plot_dat %>%
    ...
    labs(
      title = "Restricted special-unit SNV by location overlap",
      subtitle = "All location-level comparisons are restricted to pairs with overlapping admission stay anywhere in the hospital",
      x = "SNV distance",
      y = "Pairs"
    ) +
  ...
  ggsave(
    filename = file.path(viz_dir, "restricted_special_unit_overlap_snv_0_75.png"),
    plot = p,
    width = 14,
    height = 8,
    units = "in",
  ...
  save_plot_pdf(
    plot_obj = p,
    filename = file.path(viz_dir, "restricted_special_unit_overlap_snv_0_75.pdf"),
    width = 14,
    height = 8
  )
}
...
make_threshold_plot <- function(threshold_dat) {
  ...
  ann_dat <- plot_dat %>%
    ...
    summarise(y_pos = max(pct, na.rm = TRUE) * 1.15 + 0.2, star_label = first(star_label), .groups = "drop")
  p <- ggplot(plot_dat, aes(x = threshold_rule, y = pct, fill = group)) +
    geom_col(position = position_dodge(width = 0.75), width = 0.65) +
    geom_text(aes(label = sprintf("%.1f", pct)), position = position_dodge(width = 0.75), vjust = -0.2, size = 3.2) +
    geom_text(data = ann_dat, aes(x = threshold_rule, y = y_pos, label = star_label), inherit.aes = FALSE, fontface = "bold", size = 5) +
    facet_grid(clade_group ~ exposure_type, scales = "free_y") +
    scale_fill_manual(values = c("Yes" = "#e15759", "No" = "#4e79a7"), name = NULL) +
    labs(
      title = "Restricted special-unit threshold enrichment for direct overlap",
      subtitle = "All location-level comparisons are restricted to pairs with overlapping admission stay anywhere in the hospital",
      x = NULL,
  ...
  ggsave(
    filename = file.path(viz_dir, "restricted_special_unit_overlap_threshold_enrichment.png"),
    plot = p,
    width = 14,
    height = 8,
    units = "in",
  ...
  save_plot_pdf(
    plot_obj = p,
    filename = file.path(viz_dir, "restricted_special_unit_overlap_threshold_enrichment.pdf"),
    width = 14,
    height = 8
  )
}
...
plot_dat <- build_plot_data(annotated_pairs)
threshold_dat <- run_threshold_tests(plot_dat)

write.csv(
  threshold_dat,
  file.path(desc_dir, "restricted_special_unit_overlap_threshold_enrichment_fisher_tests.csv"),
  row.names = FALSE
)
write.csv(
  annotated_pairs,
  file.path(desc_dir, "restricted_special_unit_overlap_annotated_pairs.csv"),
  row.names = FALSE
)

writeLines(
  c(
    "Restricted special-unit comparison set analysis uses between-patient pairs only.",
  ...
  con = file.path(desc_dir, "restricted_special_unit_overlap_analysis_notes.txt")
)

make_distribution_plot(plot_dat)
make_threshold_plot(threshold_dat)

cat("Done. Restricted special-unit overlap outputs written to:\n", desc_dir, "\n", sep = "")
