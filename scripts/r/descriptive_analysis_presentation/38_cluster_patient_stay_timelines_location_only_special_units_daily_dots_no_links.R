...
cluster_fig_dir <- file.path(output_dir, "cluster_patient_stay_timelines_location_only_special_units_daily_dots_no_links")
...
is_special_unit <- function(x) {
  x <- trimws(as.character(x))
  is_present(x) & !grepl("^[0-9]", x)
}
...
make_unit_palette <- function(unit_values) {
  ...
  base_cols <- unikn::pal_unikn_dark
...
find_same_unit_same_day_links <- function(stay_df, patient_levels) {
  if (nrow(stay_df) < 2) {
    return(tibble(
      Unit = character(),
      patient_a = character(),
  ...
  split(stay_df, stay_df$Unit) %>%
    purrr::imap_dfr(function(unit_df, unit_name) {
      if (!is_present(unit_name) || nrow(unit_df) < 2) {
        return(tibble())
      }

      unit_df <- unit_df %>% arrange(StartDate_date, EndDate_date)
      links <- list()
      idx <- 1L

      for (i in seq_len(nrow(unit_df) - 1)) {
        for (j in seq((i + 1), nrow(unit_df))) {
          if (unit_df$patient_id[i] == unit_df$patient_id[j]) {
            next
          }

          overlap_start <- max(unit_df$StartDate_date[i], unit_df$StartDate_date[j])
          overlap_end <- min(unit_df$EndDate_date[i], unit_df$EndDate_date[j])
          ...
          if (!is.na(overlap_start) && !is.na(overlap_end) && overlap_start <= overlap_end) {
            ...
            pair_a <- min(unit_df$patient_id[i], unit_df$patient_id[j])
            pair_b <- max(unit_df$patient_id[i], unit_df$patient_id[j])
      ...
      bind_rows(links) %>%
        group_by(Unit, patient_a, patient_b) %>%
...
locations <- read.delim(locations_path, check.names = FALSE, stringsAsFactors = FALSE) %>%
  ...
  filter(
    ...
    !is.na(EndDate_date),
    is_present(Unit),
    is_special_unit(Unit),
    StartDate_date >= start_window,
    StartDate_date <= end_window
  )

global_unit_values <- unique(locations$Unit)
global_unit_values <- global_unit_values[!is.na(global_unit_values) & global_unit_values != ""]
global_unit_palette <- make_unit_palette(global_unit_values)
...
for (i in seq_len(nrow(cluster_keys))) {
  ...
  same_unit_links <- find_same_unit_same_day_links(stay_df, patient_order)
  ...
  cluster_samples_plot <- find_collection_unit(cluster_samples, stay_df) %>%
    ...
    mutate(
      ...
      collection_unit = if_else(is.na(collection_unit) | collection_unit == "", "Unknown", collection_unit)
    )
  ...
  if (nrow(same_unit_links) > 0) {
    link_summary_tables[[length(link_summary_tables) + 1]] <- same_unit_links %>%
      mutate(clade_group = clade_label, cluster_id = cluster_id_value)
  }
  ...
  plot_subtitle <- paste0(
    ...
    if (removed_patient_n > 0) paste0(" (", removed_patient_n, " patients without special-unit location data removed)") else "",
    "; special units only",
    if (nrow(same_unit_links) > 0) paste0("; ", nrow(same_unit_links), " same-unit same-day links shown") else ""
  )
  ...
  if (nrow(same_unit_links) > 0) {
    p <- p +
      geom_segment(
        data = same_unit_links,
        aes(x = link_date, xend = link_date, y = patient_plot_id_a, yend = patient_plot_id_b, color = Unit),
        linetype = "dashed",
        linewidth = 0.8,
        alpha = 0.8
      )
  ...
  p <- p +
    geom_point(
      data = stay_day_df,
      aes(x = stay_day, y = patient_plot_id, color = Unit, shape = "Patient stay"),
      size = 3.4,
      alpha = 0.6
    ) +
    geom_point(
    ...
    scale_color_manual(values = global_unit_palette, name = "Unit") +
    scale_shape_manual(
    ...
    guides(
      ...
      shape = guide_legend(order = 2, override.aes = list(color = "#4d4d4d", size = c(3.4, 3.6)))
    )

  clade_stub <- tolower(gsub(" ", "_", clade_label))
  file_stub <- paste0(clade_stub, "_cluster_", cluster_id_value, "_patient_stay_timeline_location_only_special_units_daily_dots_no_links")
...
write.csv(
  ...
  file.path(desc_dir, "cluster_patient_timeline_patient_label_lookup_location_only_special_units_daily_dots_no_links.csv"),
  row.names = FALSE
)

write.csv(
  bind_rows(link_summary_tables),
  file.path(desc_dir, "cluster_patient_same_special_unit_same_day_links_summary.csv"),
  row.names = FALSE
)
...
writeLines(
  c(
    ...
    "This location-only version removes patients without any special-unit location rows in the filtered location dataset within that window.",
    "Special units are defined here as units whose names do not start with a number.",
    "Dashed vertical lines connect patients who share the same special unit on overlapping calendar days.",
    "Point colors and the legend represent Unit, using unikn::pal_unikn_dark.",
    "Patient-stay dots are shown for every day during the stay interval.",
    "Sample collection dates are shown as black triangles."
  ),
  con = file.path(desc_dir, "cluster_patient_stay_timelines_location_only_special_units_daily_dots_no_links_notes.txt")
)

cat("Done. Cluster patient stay timeline figures for special units with daily stay dots and no links written to:\n", cluster_fig_dir, "\n", sep = "")
